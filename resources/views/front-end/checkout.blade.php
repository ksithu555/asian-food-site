<x-guest-layout>
                                                                ...
                                                                <div>
                                                                    <div class="form-check">
                                                                        <input class="form-check-input" type="radio"
                                                                            name="jack" id="flexRadioDefault2"
                                                                            {{ $buyeraddress->default == 1 ? 'checked' : '' }}>
                                                                    </div>
                                                                    <input type="hidden" id="buyeraddress_id"
                                                                        name="buyeraddress_id"
                                                                        value="{{ $buyeraddress->id }}">
                                                                    <input type="hidden" name="buyer_id"
                                                                        value="{{ $buyeraddress->userid }}">
                                                                    ...
                                                                    <ul class="delivery-address-detail">
                                                                        <li>
                                                                            <h4 class="fw-500">{{ $buyeraddress->name }}
                                                                            </h4>
                                                                        </li>

                                                                        <li>
                                                                            <p class="text-content">
                                                                                {{ $buyeraddress->post_code }}</p>
                                                                            <p class="text-content">
                                                                                {{ $buyeraddress->prefecture->name }}
                                                                            </p>
                                                                            <p class="text-content">
                                                                                {{ $buyeraddress->city }}
                                                                                {{ $buyeraddress->chome }}</p>
                                                                            <p class="text-content">
                                                                                {{ $buyeraddress->building }}
                                                                                {{ $buyeraddress->room_no }}</p>
                                                                    ...
                                                                    @if ($bankAccounts->count() > 0)
                                                                        @foreach ($bankAccounts as $key => $bankAccount)
                                                                            <div class="address-box">
                                                                                <div class="row"
                                                                                    style="background-color: rgb(215, 215, 215); height: 50px; display: flex; align-items: center;">
                                                                                    <div
                                                                                    ...
                                                                                    <span class="error"
                                                                                        style="color:red"
                                                                                        id="error-selected_bank_account"></span>
                                                                                </div>
                                                                                <div
                                                                                    class="table-responsive address-table">
                                                                                    <table class="table">
                                                                                        <tbody
                            ...
                            @foreach ($cartLists as $cartlist)
                                <ul class="summery-contain">
                                    ...
                                    <li>
                                        ...
                                        <h4>{{ $cartlist->product_name }} <span>X {{ $cartlist->quantity }}</span>
                                        </h4>
                                        @php
    ...
    <script
        src="https://www.paypal.com/sdk/js?client-id=AR2lokFqBXcd_mRdoa5LKYA9LwdB5xCN3pc8u15_Ff5fdsv3aitLYTn8mKQz-ZJ4ySHNOKSxT2jNPiad&currency=JPY">
        ...
        document.querySelectorAll('input[name="jack"]').forEach(function(radio) {
            radio.addEventListener('change', function() {
                if (this.checked) {
                    ...
                    Newbuyeraddressid = this.closest('.checkout-detail').querySelector(
                        'input[name="buyeraddress_id"]').value;
                }
            });
        });
        paypal.Buttons({
        ...
        document.getElementById('btnPayWithCash').addEventListener('click', function() {
            ...
            if (isValid) {
                ...
                $.ajax({
                    ...
                    async: false,
                    success: function(response) {
                        window.location.href = "{{ route('order_success', '') }}" + "/" + response
                            .orderId;
                    },
                    error: function(xhr, status, error) {
                        var errorMessage = xhr.status + ': ' + xhr.statusText;
