<x-auth-layout>
                                        ...
                                        <tbody>
                                            ...
                                            @else
                                                @foreach ($transfer_history as $key => $item)
                                                    <tr>
                                                        <td>{{ $ttl + 1 - ($transfer_history->firstItem() + $key) }}
                                                        </td>
                                                        ...
                                                        <td>
                                                            @if ($item->status == 0)
                                                                <select
                                                                    class="js-example-basic-single w-100 custom-select"
                                                        ...
                                                        <td>
                                                            <div class="row d-flex mx-auto">
                                                                <div
                                                                    ...
                                                                    @if ($item->adjust_amount)
                                                                        <span
                                                                            style="margin-left: 100px;font-weight:300px">¥{{ $item->adjust_amount }}
                                                                    @endif
                                                                    </span>
                                                                </div>
                                                                <div
                                                            ...
                                                            @if ($item->status == 0)
                                                                <label class="switch">
                                                                    <input data-width="100"
                                                                        ...
                                                                        data-toggle="toggle" data-on="Paid"
                                                                        data-off="Unpaid"
                                                                        {{ $item->status ? 'checked' : '' }}>
                                                                </label>
                                                            @else
    ...
    @foreach ($transfer_history as $key => $item)
        <div class="modal fade theme-modal remove-commission" id="adjustModal{{ $item->id }}" aria-hidden="true"
            tabindex="-1">{{ $item->id }}
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <form id="commission-form{{ $item->id }}" method="POST"
                        action="{{ route('updateadjust') }}" style="display:flex;">
                        @csrf
                        <div class="modal-body">
                            <div class="form-group">
                                <input type="number" class="form-control" id="adjust{{ $item->id }}"
                                    value="{{ old('adjust') ?? ($item->adjust_amount ?? '') }}" name="adjust"
                                    placeholder="Enter Adjustment Amount">
                            </div>
                        </div>
                        <div class="modal-footer">
                            <input type="hidden" id="adjustId{{ $item->id }}" name="adjustId"
                                value="{{ $item->id }}">
