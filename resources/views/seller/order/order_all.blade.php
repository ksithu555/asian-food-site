                        ...
                        <div class="card-body">
                            ...
                            <ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
                                ...
                                <li class="nav-item" role="presentation">
                                    <button class="nav-link {{ $activeTab == 'list' ? 'active' : '' }}" id="pills-home-tab"
                                        data-bs-toggle="tab" data-bs-target="#pills-home" type="button"
                                        role="tab">List</button>
                                </li>
                                <li class="nav-item" role="presentation">
                                    <button class="nav-link {{ $activeTab == 'second' ? 'active' : '' }}"
                                        id="pills-profile-tab" data-bs-toggle="tab" data-bs-target="#pills-profile"
                                        type="button" role="tab">Cancelled Order</button>
                                </li>
                            </ul>

                            <div class="tab-content" id="pills-tabContent">
                                <div class="tab-pane fade {{ $activeTab == 'list' ? 'show active' : '' }}" id="pills-home"
                                    role="tabpanel">
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td>
                                                                <ul>
                                                                    <li>
                                                                        <a
                                                                            href="{{ route('detail.order', ['id' => $item->order_id]) }}">
                                                                    ...
                                                                    <li>
                                                                        <a href="{{ route('invoice', $item->id) }}"
                                                                            @if ($item->status === 'Cancel') onclick="return false;" @endif>
                                                ...
                                                @else
                                                    @foreach ($cancelledOrder as $key => $item)
                                                        <tr>
                                                            ...
                                                            <td>{{ $item->order->order_code }}</td>
                                                            <td><a
                                                                    href="{{ route('detail.product', $item->product->id) }}">{{ $item->product->product_code }}</a>
                                                            </td>
                                                            <td>
                                ...
                                <div class="accordion-body">
                                    <form action="{{ route('order.status') }}" method="POST">
                                        ...
                                        @if ($item->status === 'Pending')
                                            ...
                                            @error('expected_to')
                                                <div class="text-danger">{{ $message }}</div>
                                            @enderror
                                            <button type="submit" class="btn btn-outline-primary w-100" name="status"
                                                value="Confirmed">Confirmed</button>
                                        @elseif ($item->status === 'Confirmed')
                                            <button type="submit" class="btn btn-outline-primary w-100" name="status"
                                                value="Processing">Processing</button>
                                        @elseif ($item->status === 'Processing')
                                            <button type="submit" class="btn btn-outline-primary w-100" name="status"
                                                value="Picked">Pick up</button>
                                        @elseif ($item->status === 'Picked')
                                            <button type="submit" class="btn btn-outline-primary w-100" name="status"
                                                value="Shipped">Shipping</button>
                                    ...
                                    @if ($item->status === 'Pending')
                                        <form action="{{ route('order.cancel') }}" method="GET">
                                            ...
                                            <input type="hidden" name="id" value="{{ $item->id }}">
                                            <button type="submit"
                                                class="btn btn-outline-primary w-100 {{ $item->status === 'Cancel' || $item->status === 'Delivered' ? 'disabled-blue' : '' }}"
                                                name="status" value="Cancel"
                                                {{ $item->status === 'Cancel' || $item->status === 'Delivered' ? 'disabled' : '' }}>Order
                                                Cancel</button>
                                        </form>
                                    @endif
                                </div>
